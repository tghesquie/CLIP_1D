...
def Clip_no_opt_algorithm(dt, parameters):
    ...
    for step in tqdm(range(0, max_steps), desc = "Time steps"):
        ...
        if parameters.new_crack != 0:
            ...
            Ecoh_verlet = (0.5 * parameters.k * np.sum(functions.gd_cohesive.get_value(d[math.floor((parameters.N_elements-1)/2)]) * np.sum(jump*prev_jump)))*parameters.Area
        ...
        # Ekin_verlet = np.sum(0.5 * mass[1:] * vel_predict * vel_predict)
        # Epot_verlet = 0.5 * Area * parameters.dx * E *(np.dot((strain * previous_strain),functions.GD_bulk.get_value(D)))
        # strain_increment = strain - previous_strain
        # stress_average = (stress_bulk + previous_stress )/2
        ...
        # bulk_dissipation_act = (bulk_dissipation - Epot_verlet)

        # previous_strain = strain
        # prev_traction = traction_at_center
        # previous_stress = stress_bulk

        # cohesive_dissipation_act = (cohesive_dissipation - Ecoh_verlet)
        # total_dissipation = bulk_dissipation_act + cohesive_dissipation_act

        # #Total_energy = Ep + Ekin  + total_dissipation + Ecoh
        # Total_energy_verlet = Epot_verlet + Ekin_verlet + Ecoh_verlet + total_dissipation
        ...
        results_dict['GD_function'].append(np.copy(functions.GD_bulk.get_value(D_nodes)))
        results_dict['strain_str'].append(np.copy(strain))
        results_dict['strain_change_str'].append(np.copy(strain_increment))
        results_dict['dissipated_energy'].append(np.copy(energies['Edissip']))
        results_dict['dissipated_bulk_energy'].append(np.copy(energies['Edissip_bulk']))
        results_dict['dissipated_coh_energy'].append(np.copy(energies['Edissip_coh']))
        results_dict['potential_energy'].append(np.copy(energies_verlet['Epot']))
...
if __name__ == "__main__" :
    ...
    eps0dot = 7
    N_elements = 160
    max_steps = int(300/40*N_elements)
    Dm = 0.6
    new_crack = 0
    boundary_type = "free"
    nlc = 16
    ...
    # #results_opti_clip_1 = Clip_opt_algorithm(results_aka['time_step'], paramters_opti_clip_1)
    ...
    # #results_opti_clip_2 = Clip_opt_algorithm(results_aka['time_step'], paramters_opti_clip_2)
    ...
    # opening_along_time(results_aka['time_step'],results_no_opti_clip)
